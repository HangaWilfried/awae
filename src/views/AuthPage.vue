      ...
      <section class="w-1/2 m-auto space-y-10 pt-10">
        <h1 class="flex flex-col items-center gap-2">
          <span class="font-bold text-2xl"
            >{{ t('sayWelcome') }}<strong class="text-blue-500">AWAE</strong></span
          >
          <span class="text-base">{{ t('doSignin') }}</span>
        </h1>
        <div class="flex flex-col gap-5">
          <TextField placeholder="" :label="t('email')" v-model="state.email" />
          <PasswordField placeholder="" :label="t('password')" v-model="state.password" />
          <div class="grid pt-10">
            <TwButton :theme="THEME.BLUE" :cta="t('signin')" @click="performAuthentication" />
          </div>
        </div>
        <div class="text-center py-7 space-x-2 border-t border-gray-300 text-sm">
